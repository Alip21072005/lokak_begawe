...
// Penamaan props disesuaikan dengan data yang dikirim Controller
const props = defineProps<{ 
    lowongan: any 
}>();

// Kita buat alias 'job' agar tidak perlu merubah banyak kode di template, 
// tapi tetap aman dengan pengecekan props.lowongan
...
// Helper untuk format Rupiah agar lebih pro
...
<template>
    ...
    <div v-else class="space-y-8 p-6 lg:p-10">
        <div class="flex items-center justify-between">
            ...
            <div v-if="job.status_lowongan !== 'pending'" 
                ...
                <div class="rounded-[2.5rem] border border-slate-200 bg-white p-10 shadow-sm">
                    ...
                    <div class="mb-10 flex flex-wrap gap-6">
                        ...
                        <span class="flex items-center gap-2 text-[10px] font-black uppercase text-emerald-600 italic">
                            <Banknote class="h-4 w-4" /> {{ formatRupiah(job.gaji_min) }}
                        </span>
                    </div>
            ...
            <div class="space-y-8 lg:col-span-5">
                <div class="rounded-[2.5rem] border border-slate-200 bg-white p-8 shadow-sm">
                    ...
                    <div v-if="job.bukti_pembayaran" class="relative group overflow-hidden rounded-3xl border border-slate-100 shadow-xl bg-slate-50">
                        <img :src="job.bukti_pembayaran" class="w-full transition-transform duration-700 group-hover:scale-110" />
                        <div class="absolute inset-0 bg-linear-to-t from-slate-900/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-6">
                            <a :href="job.bukti_pembayaran" target="_blank" class="text-[10px] font-black text-white uppercase italic underline">Lihat Full Image</a>
                        </div>
                    </div>
                    ...
                    <div class="mt-10">
                        ...
                        <div v-else-if="job.status_lowongan === 'verified'" 
                        ...
                        <div v-else-if="job.status_lowongan === 'rejected'" 
                ...
                <div class="px-8 flex items-center justify-between text-[9px] font-bold text-slate-300 uppercase italic tracking-widest">
                    <span>ID: {{ job.id }}</span>
                    <span class="flex items-center gap-1"><Clock class="h-3 w-3" /> {{ new Date(job.created_at).toLocaleDateString() }}</span>
                </div>
            </div>
        </div>
    </div>
